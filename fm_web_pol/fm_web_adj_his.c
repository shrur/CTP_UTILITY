 ...
 *  *  *      * Insanity check.
 *   *   *      ***********************************************************/
        if (opcode != WEB_OP_ADJ_HIS  ) {
                pin_set_err(ebufp, PIN_ERRLOC_FM,
                        PIN_ERRCLASS_SYSTEM_DETERMINATE,
                        PIN_ERR_BAD_OPCODE, 0, 0, opcode);
                PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
                        "WEB_OP_ADJ_HIS  opcode error", ebufp);
                return;
        }

 PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                "WEB_OP_ADJ_HIS input flist", i_flistp);

	fm_his_adjustment (ctxp,i_flistp,&final_flistp,ebufp);	
 /***********************************************************
 *  *  *          * Results.
 *   *   ************************************************************/
       if (PIN_ERR_IS_ERR(ebufp))
 ...
 *   *   *                ***************************************************/
        ...
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
               "WEB_OP_ADJ_HIS  output", *o_flistpp);
 ...
 * 1     PIN_FLD_END_T               TSTAMP [0] NULL*/
	 ...
	 db1= (poid_t *)PIN_FLIST_FLD_GET(i_flistp, PIN_FLD_POID,
        ...
        PIN_FLIST_FLD_SET(search_flistp, PIN_FLD_FLAGS, (void *)&s_flags, ebufp);
                strcpy ( s1_template, "select X from /event where F1 = V1 and F2 >V2 and event_t.poid_type like \'%%/event/billing/adjustment/account%%\' order by F2 ");
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                "fm_his_adjustment: input search flist ",
                search_flistp);
                PCM_OP(ctxp, PCM_OP_SEARCH, 0,search_flistp , &b1_flistp, ebufp);
        PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
                "fm_his_adjustment output flist",
                b1_flistp);
	...
	if(!res_flistp)
        {
                ...
                PIN_FLIST_FLD_SET(array1_flistp,PIN_FLD_NAME,"NO ADJUSTMENT HISTORY FOUND",ebufp);
                PIN_FLIST_FLD_SET(array1_flistp,PIN_FLD_AMOUNT,totalp,ebufp);
                PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
               "fm_his_adjustment: inside if",
                f_flistp);
        }
        else
        {
